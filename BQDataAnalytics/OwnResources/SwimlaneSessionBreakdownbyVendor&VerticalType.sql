...
WITH ga_vendor_table AS(
  SELECT DISTINCT
    ...
    vendor_click_details.vendor_click_origin AS swimlane_title,
  ...
  WHERE 
    ...
    AND vendor_click_details.vendor_click_origin LIKE "%swimlane%"
  GROUP BY 1,2,3,4
  ORDER BY 1 
), pd_vendor_table AS(
  SELECT 
  ...
  FROM merge_table WHERE vertical_type IS NOT NULL
), lastest_table AS(
  SELECT 
    vertical_type,
    swimlane_title,
