...
WITH close_date AS(				
    ...
    WHERE				
    is_earliest AND vendor_code IS NOT NULL
) ,zone AS(				
    SELECT				
    ...
    FROM				
        `fulfillment-dwh-production.pandata_curated.lg_countries` country,				
    UNNEST(cities) ct,				
    UNNEST(ct.zones) z				
    WHERE				
    ...
    WHERE 
        st_contains(z.zone_geo,v.location_geo) IS TRUE				
...
WHERE				
    ...
    AND v.activated_at_local >= DATE_SUB(CURRENT_DATE(),INTERVAL 30 day)	
