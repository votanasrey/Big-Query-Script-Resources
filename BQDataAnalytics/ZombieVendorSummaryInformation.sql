...
WITH data_table AS (												
    SELECT 
        ...
        v.chain_code,
        v.chain_name,												
        IFNULL(gmv.gmv_class,salesforce_accounts.gmv_class) AS gmv_class,												
        v.is_active,												
        v.is_private,												
        v.is_test,												
    ...
    LEFT JOIN `fulfillment-dwh-production.pandata_curated.pd_orders` AS o												
        ...
        AND o.created_date_utc < CURRENT_DATE AND o.created_date_local >= DATE_SUB(CURRENT_DATE(), INTERVAL 90 DAY)												
    ...
    LEFT JOIN `fulfillment-dwh-production.pandata_report.vendor_gmv_class` AS gmv												
        ON gmv.global_entity_id = 'FP_KH' 
        AND gmv.vendor_code = v.vendor_code												
    WHERE 
